...
-- Run the entire file once in the Supabase SQL editor (service role).
...
-- ============================================================
-- ENUM TYPES  (idempotent — only created if missing)
-- ============================================================
do $$
begin
...
-- ============================================================
-- HELPER FUNCTION — keeps updated_at fresh on any row update
-- ============================================================
...
-- FIX: handles username collisions gracefully (null instead of error)
-- ============================================================
...
declare
  v_username text;
begin
  -- Derive a username from the email local-part, sanitised to [a-z0-9._]
  v_username := nullif(
    lower(regexp_replace(split_part(new.email, '@', 1), '[^a-z0-9._]', '', 'g')),
    ''
  );

  -- If that username is already taken, fall back to null so the user
  -- can choose one later — avoids a unique-constraint crash at sign-up.
  if v_username is not null
     and exists (select 1 from public.profiles where username = v_username)
  then
    v_username := null;
  end if;
...
-- ============================================================
-- TRIGGER FUNCTION — keep profile in sync when auth.users changes
-- ============================================================
...
begin
  ...
  update public.profiles
     set first_name = coalesce(nullif(new.raw_user_meta_data ->> 'first_name', ''), first_name),
...
-- ============================================================
-- RPC: current user's lifetime stats
-- ============================================================
...
-- ============================================================
-- RPC: leaderboard
-- FIX: explicit parentheses; excludes zero-pace from average;
--      falls back to username/email prefix when name is blank
-- ============================================================
create or replace function public.leaderboard_distance(period text default 'all')
returns table (
  user_id             uuid,
  display_name        text,
  total_distance_m    numeric,
...
as $$
  with filtered as (
    ...
    where status = 'completed'
      ...
      and (
        period = 'all'
        or (period = 'monthly' and started_at >= timezone('utc', now()) - interval '30 days')
        or (period = 'weekly'  and started_at >= timezone('utc', now()) - interval '7 days')
      )
  )
  select
    w.user_id,
    ...
    coalesce(avg(w.avg_pace_sec_per_km) filter (where w.avg_pace_sec_per_km > 0), 0) as avg_pace_sec_per_km,
    count(distinct date(w.started_at))                                               as active_days
  from filtered w
  join public.profiles p on p.id = w.user_id
  group by w.user_id, p.first_name, p.last_name, p.username
...
create policy "private_user_data_select_own" on public.private_user_data
  for select to authenticated using (auth.uid() = user_id);

drop policy if exists "private_user_data_insert_own" on public.private_user_data;
create policy "private_user_data_insert_own" on public.private_user_data
  for insert to authenticated with check (auth.uid() = user_id);

drop policy if exists "private_user_data_update_own" on public.private_user_data;
create policy "private_user_data_update_own" on public.private_user_data
  for update to authenticated
...
-- FIX: owners and admins can update their club
...
-- FIX: creator can update their challenge (e.g., change status, extend end_at)
...
-- FIX: users can leave a challenge
...
-- Users can mark their own participation as completed
...
create policy "workout_samples_select_own" on public.workout_samples
  for select to authenticated
  using (
    exists (
      select 1 from public.workouts w
...
create policy "workout_samples_insert_own" on public.workout_samples
  for insert to authenticated
  with check (
    exists (
      select 1 from public.workouts w
...
create policy "community_messages_select_authenticated" on public.community_messages
  for select to authenticated using (true);

drop policy if exists "community_messages_insert_self" on public.community_messages;
create policy "community_messages_insert_self" on public.community_messages
  for insert to authenticated with check (auth.uid() = user_id);
...
-- ============================================================
-- SEED DATA
-- Run AFTER at least one user signs up — needs a real user_id.
...
declare
  ...
  seed_club bigint;
begin
  -- Pick the oldest user as the seed owner
  select id into seed_uid from auth.users order by created_at asc limit 1;
  if seed_uid is null then
    raise notice 'Seed skipped: no users yet. Re-run this block after first sign-up.';
    return;
  end if;
  ...
  values (
    'StrideSense Community',
    'The official StrideSense running club. Join and run with friends!',
    seed_uid
  )
  on conflict (name) do nothing;

  select id into seed_club
  from public.clubs where name = 'StrideSense Community' limit 1;

  insert into public.club_members (club_id, user_id, role)
  values (seed_club, seed_uid, 'owner')
  on conflict (club_id, user_id) do nothing;

  -- Challenges (only if table is empty)
  if (select count(*) from public.challenges) = 0 then
    insert into public.challenges
      (title, description, type, target_value, start_at, end_at, status, created_by, club_id)
    values
      ('5K Weekly Challenge',
       'Complete a total of 5 km in a single week. Every step counts!',
       'distance', 5.0,
       timezone('utc', now()),
       timezone('utc', now()) + interval '7 days',
       'active', seed_uid, seed_club),

      ('10K Monthly Distance',
       'Log 10 km of running across any number of sessions this month.',
       'distance', 10.0,
       timezone('utc', now()),
      ...
      ('30-Day Streak',
       'Log at least one run every day for 30 consecutive days.',
       'count', 30.0,
       timezone('utc', now()),
       timezone('utc', now()) + interval '30 days',
       'active', seed_uid, seed_club),
      ...
      ('Half-Marathon Prep',
       'Build up to 21.1 km total distance over the next six weeks.',
       'distance', 21.1,
       timezone('utc', now()),
       timezone('utc', now()) + interval '42 days',
       'upcoming', seed_uid, seed_club),

      ('Speed Burst',
       'Complete 5 separate runs in a single week — quality over quantity!',
       'count', 5.0,
       timezone('utc', now()),
       timezone('utc', now()) + interval '7 days',
       'active', seed_uid, seed_club);

    raise notice 'Seed: 5 challenges created.';
  else
    raise notice 'Seed: challenges already exist, skipped.';
  end if;
end $$;
